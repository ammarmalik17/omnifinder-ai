...
Your response should be informative, well-structured, and cite sources when possible."""),
    ...
    def process_query(self, query: str) -> Dict[str, Any]:
        ...
        now = datetime.now()
        self.logger.info(f"\n{'='*60}")
        self.logger.info(f"🧠 REACT AGENT STARTING at {now.strftime('%H:%M:%S.%f')[:-3]}")
        ...
        while iteration < max_iterations:
            ...
            try:
                ...
                self.logger.info(f"📝 Response content ({len(response_content)} chars):\n{response_content[:300]}..." if len(response_content) > 300 else f"📝 Response content ({len(response_content)} chars):\n{response_content}")
                ...
                if "ACTION:" in response_content.upper():
                    ...
                    if action_line:
                        ...
                        if args_line:
                            ...
                            try:
                                ...
                                self.logger.warning(f"⚠️ JSON parsing failed: {e}, using default query")
                        ...
                        if tool_name in self.tools:
                            ...
                            try:
                                ...
                                self.logger.info(f"✅ Tool execution successful in {tool_exec_duration:.3f}s (result: {len(tool_result)} chars)")
                                self.logger.log_react_iteration(iteration, "tool_executed", tool_name)
                            except Exception as e:
                                tool_exec_duration = time.time() - tool_exec_start
                                tool_result = f"Error executing {tool_name}: {str(e)}"
                                self.logger.error(f"❌ Tool execution error after {tool_exec_duration:.3f}s: {tool_result}")
                        else:
                            tool_exec_duration = time.time() - tool_exec_start
                        ...
                        self.logger.warning("⚠️ No valid action found, generating final response")
                        steps.append({
                            "step": iteration,
                    ...
                    self.logger.info("✅ No ACTION keyword found - assuming final answer")
                    steps.append({
                        "step": iteration,
                        "action": "final_response",
                        "content": response_content
