...
class TimingContext:
    ...
    def __enter__(self):
        ...
        self.logger.info(
            f"⏱️ START: {self.operation} at {self.start_datetime.strftime('%H:%M:%S.%f')[:-3]}"
        )
        if self.details:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        self.logger.info(
            f"⏱️ END: {self.operation} completed in {duration:.3f}s at {end_datetime.strftime('%H:%M:%S.%f')[:-3]}"
        )
        if exc_type:
...
class AgentLogger:
    ...
    def log_query_processing(self, query: str, method: str = "traditional"):
        ...
        timestamp = datetime.now().strftime("%H:%M:%S.%f")[:-3]
        self.logger.info(
            f"🔍 Processing query with {method} method at {timestamp}: {query[:50]}..."
        )
    ...
    def log_tool_usage(self, tool_name: str, query: str):
        ...
        timestamp = datetime.now().strftime("%H:%M:%S.%f")[:-3]
        self.logger.info(
            f"🔧 Using tool '{tool_name}' at {timestamp} for query: {query[:50]}..."
        )
    ...
    def log_tool_result(self, tool_name: str, duration: float, result_length: int = 0):
        ...
        timestamp = datetime.now().strftime("%H:%M:%S.%f")[:-3]
        self.logger.info(
            f"✅ Tool '{tool_name}' completed at {timestamp} in {duration:.3f}s (result: {result_length} chars)"
        )
    ...
    def log_tool_error(self, tool_name: str, error: str, duration: float):
        ...
        timestamp = datetime.now().strftime("%H:%M:%S.%f")[:-3]
        self.logger.error(
            f"❌ Tool '{tool_name}' failed at {timestamp} after {duration:.3f}s: {error}"
        )
    ...
    def log_search_results(self, tool_name: str, num_results: int):
        ...
        timestamp = datetime.now().strftime("%H:%M:%S.%f")[:-3]
        self.logger.info(
            f"📊 Tool '{tool_name}' returned {num_results} results at {timestamp}"
        )

    def log_classification(
        self, primary_tool: str, secondary_tools: list, confidence: float
    ):
        """Log query classification results."""
        timestamp = datetime.now().strftime("%H:%M:%S.%f")[:-3]
        tools_str = f"{primary_tool}" + (
            f" + {', '.join(secondary_tools)}" if secondary_tools else ""
        )
        self.logger.info(
            f"🎯 Classification at {timestamp}: Primary={primary_tool}, Confidence={confidence:.2f}, Tools=[{tools_str}]"
        )
    ...
    def log_synthesis_start(self, num_sources: int):
        ...
        timestamp = datetime.now().strftime("%H:%M:%S.%f")[:-3]
        self.logger.info(
            f"📝 Starting result synthesis at {timestamp} with {num_sources} sources"
        )
    ...
    def log_synthesis_complete(self, duration: float, answer_length: int):
        ...
        timestamp = datetime.now().strftime("%H:%M:%S.%f")[:-3]
        self.logger.info(
            f"✅ Synthesis completed at {timestamp} in {duration:.3f}s (answer: {answer_length} chars)"
        )
    ...
    def log_react_iteration(self, iteration: int, action: str, tool: str = None):
        ...
        self.logger.info(
            f"🔄 ReAct iteration {iteration} at {timestamp}: {action}{tool_info}"
        )
    ...
    def log_memory_operation(self, operation: str, message_count: int = 0):
        ...
        timestamp = datetime.now().strftime("%H:%M:%S.%f")[:-3]
        self.logger.info(
            f"🧠 Memory {operation} at {timestamp} (messages: {message_count})"
        )
